    ...
    for line in lines:
        if line:
            ...
            if len(line_processed) > 1:
                line_processed = [line_processed[0]] + line_processed[1].split("|")
...
def get_vectors(model, lines):
    ...
    for line in lines:
        ...
        if len(corpus) > 1:
            corpus = [corpus[0]] + corpus[1].split("|")
...
if __name__ == "__main__":
    print("Reading file ...")
    data = open(f'{RESOURCES_PATH}/apiElements_COSTER.txt')
    ...
    model_w2vec.save(f"{RESULTS_PATH}/models/text/word2vec_github_coster_ml.model")

    Path(f"{RESULTS_PATH}/models/libraries_information/").mkdir(parents=True, exist_ok=True)
    pickle.dump(libs_mapping, open(f"{RESULTS_PATH}/models/libraries_information/libs_mapping_github_coster_ml.pickle", "wb"))

    Path(f"{RESULTS_PATH}/input/").mkdir(parents=True, exist_ok=True)
    pickle.dump(X, open(f"{RESULTS_PATH}/input/X.pickle", "wb"))
    pickle.dump(y, open(f"{RESULTS_PATH}/input/y.pickle", "wb"))

    print("Getting the trained vectors in Word2vec ...")
    X = pickle.load(open(f"{RESULTS_PATH}/input/X.pickle", "rb"))
    y = pickle.load(open(f"{RESULTS_PATH}/input/y.pickle", "rb"))
    ...
    for name, classifier in models_test:
        ...
        model_filename = f"{RESULTS_PATH}/models/classifiers/{name}_github_coster_ml.model"
